...
`~/.config/enigma/index.lock` (0600; Issue #66): interprocess lock file guarding `mutateIndex`'s re-read → delta → write critical section. Body is three lines: `<token>\n<pid>\n<createdAtMs>\n` — `token` is a random 32-char hex generated fresh on each acquire (debug visibility only, never a value). Acquired with `O_EXCL`; stale (>30 s, judged from the lock's own `createdAtMs`) locks are broken via tombstone rename so only one breaker wins, with a post-rename freshness re-check that restores the lock instead of proceeding if it turns out a live process re-acquired it in the gap. Release only unlinks the file if it still carries the token it was acquired with. Bounded-retry exhaustion, and any unexpected fs error during acquisition, surfaces as `E_LOCK_TIMEOUT` naming the lock path. Callers never touch this file directly — `mutateIndex` owns its entire lifecycle.
