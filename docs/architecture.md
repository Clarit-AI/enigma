...
### Audit line format (Issue #80)

`~/.config/enigma/audit.log` is one JSONL file per user, shared by every repo — so a `scope: 'project'` line carries `projectId` (the repo-identity id the index uses, `projectId(cwd)` from Issue #67 — not a lexical path hash) and `projectPath` (in clear, exactly as `index.json` records it under D1.1). For ops on an existing index entry (`read`, `reveal`, `remove`, `move`, `leak`) the line records the **entry's** stored fields, not a recomputed value; `set`/`import` use `projectId(cwd)` for the project being written. `migrate` records the entry's own projectId — for a re-keyed entry that is already the new repo id; for a refused or pruned one it is the recorded (legacy) id. Global-scope lines carry neither field; lines written before Issue #80 omit them and remain valid — treat an absent `projectId` as "not recorded". Writes are compile-time enforced: `appendAuditEvent` takes a discriminated scope slice (`AuditEventInput`) where `scope: 'project'` requires `projectId`, and every call site with a runtime scope goes through `auditScopeFields`, which throws `E_INDEX_CORRUPT` rather than write an unattributed project line. Names-only is preserved — no value, ref, or request reason is ever written.
...
`deleteSecret` resolves and deletes from the depository *before* acquiring the lock, then removes the exact entry deleted (matched by name+scope+projectId **and** `ref`, not a fresh scope resolution) inside the lock — refusing with `E_NOT_FOUND` rather than guessing if that entry is gone or changed by the time the lock is acquired, so a same-name entry committed by a different scope during the depository await is never removed by mistake. Refs are stable addresses rather than content generations; the exact-entry match narrows the window but the residual same-name rotate/delete races remain — the same hazard class Issue #70's cleanup guards on the rotate path still applies, pre-existing, to `deleteSecret`'s and `move`'s own physical deletes. A known limitation: two concurrent `set` calls with `rotate=false` for the same name may leave the loser's value as an orphan in the depository (the index reflects the winner).

### Rotate removes the displaced copy (Issue #70)

A `setSecret` rotate that lands in the **same** depository as the displaced entry deletes the old storage location after the index commit — never inside the lock, best-effort, and only under three guards, because stable addresses (`env`'s `(projectPath, NAME)`, `<id>/NAME` elsewhere) are *reusable*: a concurrent rotate can legitimately repopulate a displaced address while the cleanup is in flight.

1. **Location differs** — the displaced `ref` differs from the new ref, or for `env` the displaced `projectPath` canonicalizes (realpath, lexical fallback) differently from the new one. Same-address writes already overwrote in place; deleting them would destroy the new value.
2. **Not reclaimed** — no committed index entry still maps to the displaced location (post-commit `readIndex`; the index is always read fully-committed since writes are atomic renames).
3. **Content unchanged** — where the depository can verify prompt-free (`env`, `encrypted`), `Depository.deleteIfUnchanged(ref, capturedOldValue)` compare-and-deletes inside the depository's own synchronous read-modify-write, covering repopulation that lands between the commit and the delete. `1password` needs no content guard (fresh item ids are never reused); `keychain`/`secret-service` get guards 1–2 only (a guarding read could prompt).

A failed cleanup never fails the rotate — the new value and index are already authoritative: a warning naming name+depository goes to the caller's `warnings`, and an audit line (`op: remove`, `ok: false`) records the `classifyCleanupError` label. No value ever reaches the warning, the audit, or argv. Residuals, by design: a same-value uncommitted rewrite racing the delete; the pre-existing cross-process `.env`/`secrets.enc` RMW (the index lock does not cover depository files); and the millisecond keychain/secret-service repopulation window. A different-depository rotate is unchanged — `move` owns that cleanup.
