...
Every index write goes through `mutateIndex(delta)` in `src/core/index-store.ts`, which holds an interprocess file lock at `<ENIGMA_HOME>/index.lock` (mode 0600) for the duration of a synchronous re-read → delta → atomic-rename critical section. The lock is created with `O_EXCL`; its body is `<token>\n<pid>\n<createdAtMs>\n`, where `token` is a random 32-char hex acquired fresh each time. A lock older (by its own `createdAtMs`) than a 30 s stale threshold is broken via a tombstone `renameSync` so only one breaker wins — the loser sees `ENOENT` on its own rename (the source already moved) and simply retries acquisition rather than surfacing a raw fs error. Because the initial staleness read and the actual rename aren't atomic, the breaker re-reads the tombstone's own content after the rename: if it turns out not-stale (another process finished and re-acquired the lock in that gap), the breaker restores it via `linkSync` and backs off instead of proceeding on a stolen live lock. `release()` only unlinks the lock file if it still carries the token it was acquired with, so a release racing a stale-break replacement never deletes the new owner's lock; if `openSync` succeeds but writing the body fails, the half-created lock is unlinked before the error is rethrown, so a transient write failure can't leak a lock. Acquisition is bounded to ~500 ms of retries before throwing `E_LOCK_TIMEOUT`; no raw `node:fs` error escapes `mutateIndex` — anything unexpected is wrapped as `E_LOCK_TIMEOUT` too. The four index writers — `setSecret`, `deleteSecret`, `move` (transitively via `setSecret(..., rotate: true)`), and `import-commit` (transitively via per-entry `setSecret`) — all reach it. Slow depository I/O (1Password prompts, keychain operations) stays outside the lock; only the in-process index read/apply/write is inside. `deleteSecret` resolves and deletes from the depository *before* acquiring the lock, then removes the exact entry deleted (matched by name+scope+projectId **and** `ref`, not a fresh scope resolution) inside the lock — refusing with `E_NOT_FOUND` rather than guessing if that entry is gone or changed by the time the lock is acquired, so a same-name entry committed by a different scope during the depository await is never removed by mistake. A known limitation: two concurrent `set` calls with `rotate=false` for the same name may leave the loser's value as an orphan in the depository (the index reflects the winner). Issue #70 (rotate cleanup) builds on this helper and is the right place to address the orphan.
