...
- D3.6 Slash commands: `/enigma:request`, `/enigma:reveal`, `/enigma:list`, `/enigma:remove`, `/enigma:doctor`, `/enigma:import`; `reveal` and `import` marked `disable-model-invocation` (user-only). `/enigma:remove` was added after the initial freeze, in Issue #79 (shipped 2026-09-23, PR #81) — it wraps the pre-existing `enigma_remove` MCP tool. It shipped **user-only as well** (`disable-model-invocation: true`), since deletion is destructive and cannot be undone; that supersedes nothing in the frozen text above, which simply predates the command. Three of the six are therefore user-only, not two.
